<template>
  <div>
    <div class="d-flex align-center mb-6">
      ...
      <h1 class="text-h5 font-weight-bold">IoT 단말기 관리</h1>
      <v-spacer></v-spacer>
    ...
    <v-card elevation="3">
      <v-card-title class="d-flex align-center py-3">
        ...
        <v-chip v-if="activeFilters" color="primary" size="small" class="ml-2" variant="outlined">
          Filtered
        </v-chip>
      </v-card-title>
      <v-divider></v-divider>
    ...
    <v-dialog v-model="showDeleteDialog" max-width="400" persistent>
      <v-card>
        ...
        <v-card-text>
          <strong>{{ currentDevice.device_name }}</strong> 단말기를 삭제 하시겠습니까?
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          ...
          <v-btn color="error" variant="flat" @click="deleteDevice"> 삭제 </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  setup() {
    ...
    // 수정된 handleStatusUpdate 함수
    const handleStatusUpdate = (device) => {
      try {
        // DeviceList에서 이미 업데이트 처리를 완료했으므로 알림만 표시
        showNotification(`${device.device_name} has been set to Repair mode`, 'success')

        // 필요시 목록 강제 새로고침
        if (deviceListRef.value && deviceListRef.value.fetchDevices) {
          deviceListRef.value.fetchDevices()
        }
      } catch (error) {
        console.error('Error handling status update:', error)
        showNotification('Failed to process status update. Please try again.', 'error')
      }
    }
